...
resource "incus_instance" "plexhocs" {
  ...
  config = {
    "cloud-init.network-config" = templatefile("tftpl/netconfig.tftpl", {
      tmpl_hostcidr = "${local.hostdb[each.key]}/${var.masklen}"
      tmpl_gateway  = "${local.hostdb[local.gatebyplex[each.value.plex]]}"
    })

    # bake-time user-data gets generated by ansible cloudinit role.  for nodes
    # instantiated from baked images, we only set hostname and ssh key, except
    # kubernetes nodes, which do a kubeadm init or join depending on node plane
    #
    "cloud-init.user-data" = templatefile("tftpl/userdata.tftpl", {
      tmpl_node      = each.key
